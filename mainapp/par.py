...
class parse():	
	def parse_main(self):
		...
		sql_t.execute('DELETE FROM classroom_db')
		db_t.commit()
		...
		for i in range(len(gr1)):
			...
			gr.append (gr1[i][1])
			sql.execute(f"SELECT * FROM mainapp_name_group WHERE group_id = '{gr1[i][0]}'")
			grup = sql.fetchone()
			print (grup)
			if grup == None:
		...
		delete_table = 'DELETE FROM mainapp_lesson'
		sql.execute(delete_table)
		db.commit()
		for value in sql.execute("SELECT * FROM mainapp_lesson"):
			print (value)
					...
					while npara<5:
						...
						if raspis[npara]['less'] != 'Немає':
							...
							if 	item != -1:
								...
								sql.execute(f"INSERT INTO mainapp_lesson VALUES (?, ?, ?, ?, ?,?, ?, ?, ?)", (None, date, group_norm, npara+1, raspis[npara]['less'], prepod1, day, raspis[npara]['repl'],  '-'))
								db.commit()
								
								sql.execute(f"INSERT INTO mainapp_lesson VALUES (?, ?, ?, ?, ?,?, ?, ?, ?)", (None, date, group_norm, npara+1, raspis[npara]['less'], prepod2, day, raspis[npara]['repl'],  '-'))
								db.commit()
